  ...
  return (
    <div className="container mt-5">
      ...
      <div>
        ...
        />
        <button onClick={handleAddTime}>등록</button>
      </div>
      {timeList.map((timeList) => (
        <div className="card" key={timeList.clock2No}>
          <span>{timeList.clock2Select}</span> (<span>{timeList.clock2EatingTime}</span>)
          <button onClick={() => handleDeleteTime(timeList)}>삭제</button>
        </div>
      ))}
    </div>
  );
